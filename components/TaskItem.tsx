...
export default function TaskItem({ task }: { task: Task }) {
  ...
  if (editing) {
    return (
      <form action={onUpdate} className="rounded-xl bg-white p-4 shadow-sm ring-1 ring-slate-200">
    ...
    <div className="rounded-xl bg-white p-4 shadow-sm ring-1 ring-slate-200">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <h3 className={`font-medium text-slate-900 ${task.status === "done" ? "line-through opacity-60" : ""}`}>
            {task.title}
          </h3>
